...
public class AndroidTicTacToe extends Activity {
	...
	private void endGame(int winner) {
		if (winner == 1) {
			...
			playSound(R.raw.tie_game);
			 ...
			 playSound(R.raw.human_win); // new method, to check mSoundOn and play sound
		}
		else {
			mComputerWins++;
			mComputerScoreTextView.setText(Integer.toString(mComputerWins));
			mInfoTextView.setText(R.string.result_computer_wins);
			playSound(R.raw.computer_win);
		}
		mGameOver = true;
	}
	...
	private void playSound(Integer sound) {
		if(mSoundOn)
			mSounds.play(mSoundIDMap.get(sound), 1, 1, 1, 0, 1);
	}
